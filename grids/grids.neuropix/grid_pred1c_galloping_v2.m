function grid = grid_pred1c_galloping_v2()

% controlling the sound presentation
grid.sampleRate = tdt100k;
grid.stimGenerationFunctionName = 'stimgen_loadSoundFile';
grid.stimDir = 'e:\benware.sounds.uncalib\pred1c_galloping_v2\';
grid.stimFilename = 'pred1c_galloping_v2.type%1.set%2.n%3.wav';
...
sG1 = createPermutationGrid(3,1,1:10);
stimGrid{1} = sG1(randperm(size(sG1,1)),:);

sG2 = createPermutationGrid(4,1,1:10);
stimGrid{2} = sG2(randperm(size(sG2,1)),:);

sG3 = createPermutationGrid(3,2,1:10);
stimGrid{3} = sG3(randperm(size(sG3,1)),:);

sG4 = createPermutationGrid(4,2,1:10);
stimGrid{4} = sG4(randperm(size(sG4,1)),:);

sG5 = createPermutationGrid(3,3,1:10);
stimGrid{5} = sG5(randperm(size(sG5,1)),:);

sG6 = createPermutationGrid(4,3,1:10);
stimGrid{6} = sG6(randperm(size(sG6,1)),:);

sG7 = createPermutationGrid(3,4,1:10);
stimGrid{7} = sG7(randperm(size(sG7,1)),:);

sG8 = createPermutationGrid(4,4,1:10);
