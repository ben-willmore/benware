function grid = grid_noise_with_whisk()
  ...
  levels = 80%[60 80]
  noise_delay = 500%[50,100,125,150,200,250,350];
  whiskDelay = 100%[50,75,92,125,150,200,250,300,350,650];
  voltages = [0 8]; 
  %Create no light condition
  no_whisk_condition=createPermutationGrid(1250, 500, 100, voltages(1), whiskDelay, 50, levels);
  %Create light condition
  whisk_grid=createPermutationGrid(1250, noise_delay, 100, voltages(2), whiskDelay, 50, levels);
 ...
 % whisk_only=[1250, 500, 10, voltages(2), 100, 25, 0]
  ...
  grid.stimGrid =   whisk_grid%stim_Grid_temp% cat(1,continuous_whisk,stim_Grid_temp)%cat(1,whisk_only,stim_grid_temp2);
